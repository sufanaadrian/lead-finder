...
- Buton **WhatsApp** care deschide direct **aplicația** (nu tab-ul web) cu mesaj pre-completat (`{nume}`, `{tip}` și `{eu}` înlocuite automat) — la apăsare marchează lead-ul „Contactat"
- **Mesajul e partajat** între voi (salvat în baza de date) — îl editează oricare dintre voi din „✏️ Mesaj" și se actualizează la celălalt automat
...
2. În proiect, **SQL Editor → New query**, lipește tot conținutul fișierului [`supabase/schema.sql`](supabase/schema.sql) și apasă **Run**. Asta creează tabelele, regulile de securitate și activează Realtime pe `leads` și `app_settings`.
...
**6. Dacă proiectul Supabase a fost creat înainte de mesajul partajat**, rulează o singură dată în SQL Editor și conținutul din [`supabase/migrations/0003_settings.sql`](supabase/migrations/0003_settings.sql) (proiectele noi le au deja din `schema.sql`).
...
> La prima utilizare, fiecare dintre voi va alege „Cine ești?" (Adrian sau Andreea) dintr-un popup — ales o dată per device, salvat în browser, folosit pentru claim-uri și atribuire (cine a contactat / cine a scris notița / semnătura din mesaj), nu un cont real. Se schimbă oricând din butonul „👤" de sus.
